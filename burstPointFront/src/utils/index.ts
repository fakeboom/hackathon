...
import { ChainId, JSBI, Percent, Token, CurrencyAmount, Currency, ETHER, ROUTER_ADDRESS } from '@liuxingfeiyu/zoo-sdk'
...
const builders = {
  ...
  okchain: (chainName: string, data: string, type: 'transaction' | 'token' | 'address' | 'block') => {
  ...
  okchaintest: (chainName: string, data: string, type: 'transaction' | 'token' | 'address' | 'block') => {
...
const chains: ChainObject = {
  ...
  [ChainId.OKCHAIN_TEST]: {
    chainName: 'okchain_test',
    builder: builders.okchaintest
  },
  [ChainId.OKCHAIN]: {
    chainName: 'okchain',
    builder: builders.okchaintest
...
export function getRouterContract(chainId: number, library: Web3Provider, account?: string): Contract {
  return getContract(getRouterAddress(chainId), IUniswapV2Router02ABI.abi , library, account)
}
